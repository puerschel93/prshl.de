...
export const Decoration: FC = () => {
	...
	return (
		<Fragment>
			...
			<motion.div
				className="sm:block top-0 left-0 absolute hidden bg-primaryGray-800 w-screen h-[1px]"
				style={{ opacity: DECORATION_OPACITY }}
				animate={{ translateY: position.top - DECORATION_OFFSET }}
				transition={{ duration: 0, ease: "linear" }}
			/>
			<motion.div
				className="sm:block top-0 left-0 fixed hidden bg-primaryGray-800 w-[1px] h-screen"
				animate={{
					translateX: position.left - DECORATION_OFFSET * 1.5,
				}}
				style={{ opacity: DECORATION_OPACITY }}
				transition={{ duration: 0, ease: "linear" }}
			/>
